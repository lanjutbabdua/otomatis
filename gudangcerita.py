...
API_BASE_URL_SELF_HOSTED = "https://ngesex.org/wp-json/wp/v2/posts"

# Ganti langsung dengan blog identifier WordPress.com kamu
# Contoh: "gudangceritahot.wordpress.com" atau "yourcustomdomain.com"
WORDPRESS_COM_BLOG_IDENTIFIER = "gudangceritahot.wordpress.com" 
...
WORDPRESS_COM_ACCESS_TOKEN = "BL(cv)U5A2b*30E3VK5eDC((A)949fvq2QYDWrIGWa@5QbbZ1sIDSAzzx2ip2yTw"
...
def insert_more_tag(content_html, word_limit=100):
    """
    Menyisipkan placeholder <!--more--> di sekitar batas kata yang ditentukan dalam konten HTML.
    ...
    if insert_pos != -1:
        # --- PERBAIKAN DI SINI: Menyisipkan placeholder <!--more--> ---
...
def wrap_content_in_details_tag(content_html, article_url, article_title, word_limit=850):
    """
    ...
    if safe_insert_pos != -1:
        ...
        details_summary_html = f"<summary>Lanjut bab 2: <a href='https://otieu.com/4/9586242' rel='nofollow' target='_blank'>{escaped_title}</a></summary>"
...
def publish_post_to_wordpress_com(access_token, blog_identifier, title, content_html, date_gmt, categories=None, tags=None, random_image_url=None, article_url_for_details=None, article_title_for_details=None):
    """
    Menerbitkan postingan ke WordPress.com.
    Menambahkan parameter date_gmt untuk mengatur tanggal publikasi.
    """
    print(f"🚀 Menerbitkan '{title}' ke WordPress.com dengan tanggal: {date_gmt} (UTC)...")
    ...
    # --- PENYISIPAN BRANDING DI SINI ---
    # Buat string branding
    branding_text = f"<strong>Gudang Cerita Hot</strong> - {title}."
    
    # Cari paragraf pertama (biasanya diawali <p>)
    # Gunakan regex untuk menemukan tag <p> pembuka pertama dan sisipkan teks di dalamnya
    # Pastikan ini hanya di paragraf pertama!
    modified_content_html = re.sub(r'(<p[^>]*>)', r'\1' + branding_text, content_html, 1, re.IGNORECASE)
    
    # Jika tidak ada tag <p> sama sekali di awal, mungkin kontennya dimulai dengan plain text atau tag lain.
    # Dalam kasus ini, sisipkan di awal konten.
    if not modified_content_html or modified_content_html == content_html:
        modified_content_html = f"<p>{branding_text}</p>" + content_html
    # --- AKHIR PENYISIPAN BRANDING ---

    # 1. SISIPKAN <!--more--> DI SINI (jika diperlukan)
    content_after_more_tag = insert_more_tag(modified_content_html, word_limit=100)
    ...
    payload = {
        ...
        'date_gmt': date_gmt # Menggunakan tanggal yang kita generate sendiri
    }
...
def fetch_all_and_process_posts_from_self_hosted():
    ...
    while True:
        params = {
            ...
            'page': page,
            'status': 'publish',
            '_fields': 'id,title,content,excerpt,categories,tags,date,date_gmt,featured_media,link' 
        }
        try:
    ...
    for post in all_posts_raw:
        ...
        post['date_gmt_original_source'] = post.get('date_gmt') 
...
if __name__ == '__main__':
    ...
    print("📝 Menyisipkan placeholder <!--more--> di sekitar 100 kata pertama setiap artikel untuk penggantian manual.") # Perbarui pesan
    print("🔽 Menyisipkan <details> dengan link judul artikel sumber setelah 700 kata pertama setiap artikel.")
    print("📅 Artikel akan dipublikasikan dengan tanggal mundur harian dari hari ini.")
    print("✨ Menyisipkan branding 'Gudang Cerita Hot - [Judul Artikel].' di awal paragraf pertama.") # Update pesan
    ...
    try:
        ...
        # 5. Urutkan postingan yang belum diterbitkan (misalnya, dari yang TERBARU ke TERLAMA dari sumber,
        # meskipun tanggal ini TIDAK akan digunakan untuk publikasi, hanya untuk urutan internal)
        # Ini penting agar jika ada artikel yang sama, urutannya konsisten.
        # Atau, Anda bisa mengurutkan secara acak jika tidak peduli urutan dari sumber.
        unpublished_posts.sort(key=lambda x: datetime.datetime.fromisoformat(x['date_gmt_original_source'].replace('Z', '+00:00')), reverse=True)
        print(f"Mengurutkan {len(unpublished_posts)} artikel (internal) dari yang terbaru ke terlama dari sumber.")

        successful_publications = 0
        failed_publications = 0

        # --- LOGIKA PENENTUAN TANGGAL BARU DI SINI ---
        current_publish_date = datetime.datetime.now(datetime.timezone.utc) 
        print(f"Tanggal mulai publikasi mundur harian: {current_publish_date.strftime('%Y-%m-%d %H:%M:%S UTC')}")
        # --- END LOGIKA PENENTUAN TANGGAL BARU ---
        ...
        for post_to_publish in unpublished_posts:
            ...
            source_article_url = post_to_publish.get('source_link', '#') 
            final_processed_content = post_to_publish['raw_cleaned_content']

            date_to_publish_gmt = current_publish_date.isoformat(timespec='seconds') + 'Z' 
            ...
            print(f"\n🌟 Memproses artikel: '{processed_title}' (ID Sumber: {original_post_id}, Akan dipublikasikan pada: {date_to_publish_gmt})")
            ...
            if published_response:
                ...
                current_publish_date -= datetime.timedelta(days=1)
            else:
