...
def edit_first_300_words_with_gemini(post_id, post_title, full_text_content):
    ...
    if len(words) < 50:
        ...
        # Mengembalikan seluruh konten yang dibersihkan, dan teks tersebut sebagai konteks (karena tidak ada edit AI)
...
def load_published_posts_state():
    ...
    Akan mengembalikan set kosong jika file tidak ada atau ada error.
    """
    if os.path.exists(STATE_FILE):
        with open(STATE_FILE, 'r') as f:
            try:
                # Coba muat JSON. Jika berhasil, konversi ke set dan kembalikan.
                data = json.load(f)
                # Tambahan validasi: Pastikan datanya adalah list sebelum konversi ke set
                if isinstance(data, list): 
                    return set(data)
                else:
                    # Jika bukan list, cetak peringatan dan kembalikan set kosong
                    print(f"Warning: {STATE_FILE} content is not a list. Starting with an empty published posts list.")
                    return set()
            except json.JSONDecodeError:
                # Jika ada masalah saat mengurai JSON (file kosong/rusak), cetak peringatan dan kembalikan set kosong.
